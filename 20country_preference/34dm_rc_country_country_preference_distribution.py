...
#创建临时表, 主播国家列表
#主播国家id-主播国家name
#tmp_rc_user_country_streamer_country_pair
sql2 = """
select
...
sql10 = """
CREATE TABLE IF NOT EXISTS rc_algo.da_rc_country_country_preference_all
(
...
#写入表
sql11 = """
INSERT OVERWRITE TABLE rc_algo.da_rc_country_country_preference_all PARTITION (dt='{0}')
select
  t1.user_country_id,
...
(
  ...
  from tmp_rc_user_country_streamer_country_pair
)t1
left join
(
  select
...
#主播id-主播国家-频次-时长-金额-最近一次播出时间-原值-分数-rfm综合分数-rfm综合排名-rfm综合百分比-等级
sql20 = """
CREATE TABLE IF NOT EXISTS rc_algo.dm_rc_country_country_preference_distribution
(
user_country_id string comment '用户国家id',
user_country_name string comment '用户国家',
is_user_vip string comment '是否vip',
...
#写入表
sql21 = """
INSERT OVERWRITE TABLE rc_algo.dm_rc_country_country_preference_distribution PARTITION (dt='{0}')
select
  t1.user_country_id,
  t1.user_country_name,
  t1.is_user_vip,
...
(
  ...
  streamer_country_name,
  preference
  from rc_algo.da_rc_country_country_preference_all
  where dt='{0}'
)t1
left join
(
...
if __name__ == "__main__":
    feature_category = 'dm_rc_country_country_preference_distribution'
    spark = SparkSession.builder.enableHiveSupport().appName(feature_category).getOrCreate()
    
    #tmp_rc_user_country_list
    tmp_rc_user_country_list = spark.sql(sql0)
    tmp_rc_user_country_list.createOrReplaceTempView("tmp_rc_user_country_list")
    ...
    #tmp_rc_user_country_streamer_country_pair
    tmp_rc_user_country_streamer_country_pair = spark.sql(sql2)
    tmp_rc_user_country_streamer_country_pair.createOrReplaceTempView("tmp_rc_user_country_streamer_country_pair")


    #da_rc_country_country_preference_all
    spark.sql(sql10)
    spark.sql(sql11)
    print("***finish rc_algo.da_rc_country_country_preference_all***")

    #dm_rc_country_country_preference_distribution
    spark.sql(sql20)
    spark.sql(sql21)
    print("***finish rc_algo.dm_rc_country_country_preference_distribution***")
