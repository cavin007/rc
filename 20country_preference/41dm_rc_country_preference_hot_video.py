...
#tmp_country_country_hot_video_base
sql0 =  """
SELECT
...
sql10 = """
CREATE TABLE IF NOT EXISTS rc_algo.dm_user_country_streamer_country_hot_video_preference_detail
(
...
#写入表
sql11 = """
INSERT OVERWRITE TABLE rc_algo.dm_user_country_streamer_country_hot_video_preference_detail PARTITION (dt='{0}')
SELECT  * 
from tmp_country_country_hot_video_base
""".format(TODAY,DELDAY)


if __name__ == "__main__":
    feature_category = 'dm_user_country_streamer_country_hot_video_preference_detail'
    spark = SparkSession.builder.enableHiveSupport().appName(feature_category).getOrCreate()
    
    #tmp_country_country_hot_video_base
    tmp_country_country_hot_video_base = spark.sql(sql0)
    tmp_country_country_hot_video_base.createOrReplaceTempView("tmp_country_country_hot_video_base")

    #数据写入hive
    spark.sql(sql10)
    spark.sql(sql11)
    print("***finish rc_algo.dm_user_country_streamer_country_hot_video_preference_detail***")
