...
#tmp_country_country_hot_video_base_rnk
sql0 =  """
SELECT
  user_country_name,
  ...
  raw_score,
  row_number() over (partition by user_country_name,if_vip order by raw_score) as hotvideo_combination_score,
  row_number() over (partition by user_country_name,if_vip order by raw_score desc) as hotvideo_rnk
from rc_algo.dm_user_country_streamer_country_hot_video_preference_detail 
where dt='{0}'
""".format(TODAY,DELDAY)


#tmp_country_country_hot_video_base_rnk_min_max
sql1 =  """
SELECT
user_country_name,
...
min(raw_score) as hotvideo_raw_score_min,
max(raw_score) as hotvideo_raw_score_max
from tmp_country_country_hot_video_base_rnk
group by user_country_name
""".format(TODAY,DELDAY)


#tmp_country_country_hot_video_base_score
sql2 =  """
SELECT
    ...
    t1.hotvideo_rnk,
    round(1+(5-1)*(t1.hotvideo_combination_score-t2.hotvideo_combination_score_min)/(t2.hotvideo_combination_score_max-t2.hotvideo_combination_score_min),2) as hotvideo_rank_score_norm,
    round(1+(5-1)*(t1.hotvideo_raw_score-t2.hotvideo_raw_score_min)/(t2.hotvideo_raw_score_max-t2.hotvideo_raw_score_min),2) as hotvideo_raw_score_norm,
    t1.hotvideo_raw_score,
...
(
  SELECT
    t0.user_country_name,
    t0.user_country_name_ab,
    t0.user_region_name,
    t0.if_vip,
    ...
    t0.raw_score as hotvideo_raw_score,
    hotvideo_combination_score,
    hotvideo_rnk
  from tmp_country_country_hot_video_base_rnk t0
)t1
left join
(
  SELECT
  ...
  hotvideo_raw_score_min,
  hotvideo_raw_score_max
  from tmp_country_country_hot_video_base_rnk_min_max
)t2
...
sql10 = """
CREATE TABLE IF NOT EXISTS rc_algo.dm_user_country_streamer_country_hot_video_score
(
  ...
  hotvideo_rank_score_norm double,
  hotvideo_raw_score_norm double,
  hotvideo_raw_score double,
...
#写入表
sql11 = """
INSERT OVERWRITE TABLE rc_algo.dm_user_country_streamer_country_hot_video_score PARTITION (dt='{0}')
SELECT  
  user_country_name ,
  user_country_name_ab ,
  user_region_name ,
  ...
  hotvideo_rnk ,
  hotvideo_raw_score_norm as hotvideo_score ,
  hotvideo_rank_score_norm,
  hotvideo_raw_score_norm,
  hotvideo_raw_score ,
  hotvideo_combination_score ,
  hotvideo_combination_score_min ,
  hotvideo_combination_score_max 

from tmp_country_country_hot_video_base_score
""".format(TODAY,DELDAY)


if __name__ == "__main__":
    feature_category = 'dm_user_country_streamer_country_hot_video_score'
    spark = SparkSession.builder.enableHiveSupport().appName(feature_category).getOrCreate()
    
    #tmp_country_country_hot_video_base_rnk
    tmp_country_country_hot_video_base_rnk = spark.sql(sql0)
    tmp_country_country_hot_video_base_rnk.createOrReplaceTempView("tmp_country_country_hot_video_base_rnk")

    #tmp_country_country_hot_video_base_rnk_min_max
    tmp_country_country_hot_video_base_rnk_min_max = spark.sql(sql1)
    tmp_country_country_hot_video_base_rnk_min_max.createOrReplaceTempView("tmp_country_country_hot_video_base_rnk_min_max")

    #tmp_country_country_hot_video_base_score
    tmp_country_country_hot_video_base_score = spark.sql(sql2)
    tmp_country_country_hot_video_base_score.createOrReplaceTempView("tmp_country_country_hot_video_base_score")


    #数据写入hive
    spark.sql(sql10)
    spark.sql(sql11)
    print("***finish rc_algo.dm_user_country_streamer_country_hot_video_score***")
