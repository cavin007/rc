...
sql1 = """
CREATE TABLE IF NOT EXISTS rc_algo.dim_user_region_streamer_country_call_info_base_select
(
...
#写入表
sql2 = """
INSERT OVERWRITE TABLE rc_algo.dim_user_region_streamer_country_call_info_base_select PARTITION (dt='{0}')
SELECT
  a0.user_id,a0.user_country_id,a0.user_country_name,a0.user_region_name,a0.is_user_vip,
...
(
  SELECT
  t0.user_id,t0.user_country_id,t0.user_country_name,t1.user_region_name,t0.is_user_vip,
  ...
  (
    SELECT
      user_id,user_country_id,user_country_name,user_region_name,is_user_vip,
  ...
  left join
  (
    SELECT user_id,user_region_name,is_user_vip
    from rc_algo.dm_region_user_level_detail
    where (dt='{0}' 
          and rfm_rnk_percentage>=0.1 
          and rfm_rnk_percentage<=0.6
          ) 
    group by user_id,user_region_name,is_user_vip
  )t1
  on (t0.user_id=t1.user_id  
      and t0.user_region_name=t1.user_region_name 
...
if __name__ == "__main__":
    feature_category = 'dim_user_region_streamer_country_call_info_base_select'
    spark = SparkSession.builder.enableHiveSupport().appName(feature_category).getOrCreate()
    #数据写入hive
    spark.sql(sql1)
    spark.sql(sql2)
    print("***finish rc_algo.dim_user_region_streamer_country_call_info_base_select***")
