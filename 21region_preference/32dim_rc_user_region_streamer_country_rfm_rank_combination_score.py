...
#tmp_rc_user_region_streamer_country_rfm_individual_value
sql0 = """
select
          user_region_name,
          is_user_vip,
...
(
  ...
  from rc_algo.dim_user_region_streamer_country_call_info_base_select
  where dt = '{0}'
)t
group by 
          user_region_name,
...
#tmp_rc_user_region_streamer_country_rfm_individual_rnk
sql1 = """
select 
  ...
  from tmp_rc_user_region_streamer_country_rfm_individual_value
""".format(TODAY)

#创建临时表, RFM各子项排序
#用户国家-是否vip-主播国家-频次-金额-最近播出原值-排序
#tmp_rc_user_region_streamer_country_rfm_combination_rnk_score
sql2 = """
select 
          user_region_name,is_user_vip,
          streamer_country_id,streamer_country_name,
  ...
  from tmp_rc_user_region_streamer_country_rfm_individual_rnk
""".format(TODAY)

#创建临时表, RFM各子项排序最大值
#用户国家-是否vip-主播国家-频次分数-金额分数-最近播出最大值
#tmp_rc_user_region_streamer_country_rfm_combination_rnk_max_min
sql3 = """
  select 
          is_user_vip,
          max(rfm_combination_score) as rfm_combination_score_max,
          min(rfm_combination_score) as rfm_combination_score_min
  from tmp_rc_user_region_streamer_country_rfm_combination_rnk_score
  group by is_user_vip
""".format(TODAY)


#######
#创建临时表, RFM各子项分数
#tmp_rc_user_region_streamer_country_rfm_combination_score_all
...
(
  ...
  from tmp_rc_user_region_streamer_country_rfm_combination_rnk_max_min
)t2
...
sql10 = """
CREATE TABLE IF NOT EXISTS rc_algo.dim_rc_user_region_streamer_country_rfm_rank_combination_score
(
...
#写入表
sql11 = """
INSERT OVERWRITE TABLE rc_algo.dim_rc_user_region_streamer_country_rfm_rank_combination_score PARTITION (dt='{0}')
SELECT
user_region_name,is_user_vip,
streamer_country_id,streamer_country_name,
...
from tmp_rc_user_region_streamer_country_rfm_combination_score_all
""".format(TODAY,DELDAY)


if __name__ == "__main__":
    feature_category = 'dim_rc_user_region_streamer_country_rfm_rank_combination_score'
    spark = SparkSession.builder.enableHiveSupport().appName(feature_category).getOrCreate()
    
    #tmp_rc_user_region_streamer_country_rfm_individual_value
    tmp_rc_user_region_streamer_country_rfm_individual_value = spark.sql(sql0)
    tmp_rc_user_region_streamer_country_rfm_individual_value.createOrReplaceTempView("tmp_rc_user_region_streamer_country_rfm_individual_value")
    tmp_rc_user_region_streamer_country_rfm_individual_value.show(10)

    #tmp_rc_user_region_streamer_country_rfm_individual_rnk
    tmp_rc_user_region_streamer_country_rfm_individual_rnk = spark.sql(sql1)
    tmp_rc_user_region_streamer_country_rfm_individual_rnk.createOrReplaceTempView("tmp_rc_user_region_streamer_country_rfm_individual_rnk")
    tmp_rc_user_region_streamer_country_rfm_individual_rnk.show(10)

    #tmp_rc_user_region_streamer_country_rfm_combination_rnk_score
    tmp_rc_user_region_streamer_country_rfm_combination_rnk_score = spark.sql(sql2)
    tmp_rc_user_region_streamer_country_rfm_combination_rnk_score.createOrReplaceTempView("tmp_rc_user_region_streamer_country_rfm_combination_rnk_score")
    tmp_rc_user_region_streamer_country_rfm_combination_rnk_score.show(10)

    #tmp_rc_user_region_streamer_country_rfm_combination_rnk_max_min
    tmp_rc_user_region_streamer_country_rfm_combination_rnk_max_min = spark.sql(sql3)
    tmp_rc_user_region_streamer_country_rfm_combination_rnk_max_min.createOrReplaceTempView("tmp_rc_user_region_streamer_country_rfm_combination_rnk_max_min")
    tmp_rc_user_region_streamer_country_rfm_combination_rnk_max_min.show(10)

    #tmp_rc_user_region_streamer_country_rfm_combination_score_all
    tmp_rc_user_region_streamer_country_rfm_combination_score_all = spark.sql(sql4)
    tmp_rc_user_region_streamer_country_rfm_combination_score_all.createOrReplaceTempView("tmp_rc_user_region_streamer_country_rfm_combination_score_all")
    tmp_rc_user_region_streamer_country_rfm_combination_score_all.show(10)

    #数据写入hive
    spark.sql(sql10)
    spark.sql(sql11)
    print("***finish rc_algo.dim_rc_user_region_streamer_country_rfm_rank_combination_score ***")
