...
#tmp_region_country_hot_video_score_integration
sql0 =  """
select
  t2.user_region_name      ,
  t1.if_vip,
  ...
  t2.hotvideo_score    ,
  t2.raw_score as raw_hotvideo_score,
...
(
  ...
  hotvideo_score,
  raw_score                                                                               
  from rc_algo.dm_user_region_streamer_country_hot_video_score
  where dt='{0}'
  group by 
  user_region_name      ,                                    
  if_vip                ,                                    
...
#过滤规则
#tmp_rc_region_country_recall_restrict_rule_hotvideo
sql1 = """
select
...
sql10 = """
CREATE TABLE IF NOT EXISTS rc_algo.dm_region_country_hot_video_score_integration
(
...
#写入表 region-country-final score
sql11 = """
INSERT OVERWRITE TABLE rc_algo.dm_region_country_hot_video_score_integration PARTITION (dt='{0}')
SELECT  
  user_region_name,
  if_vip,
  ...
  avg_video_duration,
  success_rate_hotvideo
from tmp_region_country_hot_video_score_integration
""".format(TODAY,DELDAY)

#创建表结构region-country-final score-detail
#用户-用户国家-是否vip-主播id-主播国家-频次-时长-金额-最近一次播出时间
sql20 = """
CREATE TABLE IF NOT EXISTS rc_algo.dm_region_country_hot_video_score_integration_detail
(
  user_region_name string,
  if_vip string,
...
#写入表region-country-final score-detail
sql21 = """
INSERT OVERWRITE TABLE rc_algo.dm_region_country_hot_video_score_integration_detail PARTITION (dt='{0}')
SELECT  
  user_region_name ,
  if_vip ,
...
#创建表结构region-country-final score-filtering
#用户-用户国家-是否vip-主播id-主播国家-频次-时长-金额-最近一次播出时间
sql30 = """
CREATE TABLE IF NOT EXISTS rc_algo.dm_region_country_hot_video_score_integration_detail_filtering
(
  user_region_name string,
  if_vip string,
...
#写入表region-country-final score-filtering
sql31 = """
INSERT OVERWRITE TABLE rc_algo.dm_region_country_hot_video_score_integration_detail_filtering PARTITION (dt='{0}')
select
  t1.user_region_name ,
  t1.if_vip ,
...
SELECT  
  ...
  success_rate_both     ,                                    
  success_rate_hotvideo    
  from rc_algo.dm_region_country_hot_video_score_integration_detail
  where dt='{0}'
)t1
join
(
  select
      user_region_name,
      streamer_country_name
  from tmp_rc_region_country_recall_restrict_rule_hotvideo
  group by 
      user_region_name,
      streamer_country_name
)t2
...
if __name__ == "__main__":
    feature_category = 'dm_region_country_hot_video_score_integration'
    spark = SparkSession.builder.enableHiveSupport().appName(feature_category).getOrCreate()
    
    #tmp_region_country_hot_video_score_integration
    tmp_region_country_hot_video_score_integration = spark.sql(sql0)
    tmp_region_country_hot_video_score_integration.createOrReplaceTempView("tmp_region_country_hot_video_score_integration")

    #tmp_rc_region_country_recall_restrict_rule_hotvideo
    tmp_rc_region_country_recall_restrict_rule_hotvideo = spark.sql(sql1)
    tmp_rc_region_country_recall_restrict_rule_hotvideo.createOrReplaceTempView("tmp_rc_region_country_recall_restrict_rule_hotvideo")

    #数据写入hive
    spark.sql(sql10)
    spark.sql(sql11)
    print("***")
    print("***finish rc_algo.dm_region_country_hot_video_score_integration***")
    print("***")
    ...
    spark.sql(sql21)
    print("***")
    print("***finish rc_algo.dm_region_country_hot_video_score_integration_detail***")
    print("***")
    ...
    spark.sql(sql31)
    print("***")
    print("***finish rc_algo.dm_region_country_hot_video_score_integration_detail_filtering***")
    print("***")
