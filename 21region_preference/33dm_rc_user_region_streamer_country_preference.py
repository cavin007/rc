...
#tmp_rc_user_region_streamer_country_rfm_rnk_max
sql1 = """
  select 
  is_user_vip,
  max(rfm_rnk) as rfm_rnk_max
  from rc_algo.dim_rc_user_region_streamer_country_rfm_rank_combination_score
  where dt='{0}'
  group by 
  is_user_vip
""".format(TODAY)
...
#######
#创建临时表, RFM各子项分数
#tmp_rc_user_region_streamer_country_rfm_rnk_percentage
...
(
  select 
    is_user_vip,
    rfm_rnk_max  
  from tmp_rc_user_region_streamer_country_rfm_rnk_max
)t2
...
sql10 = """
CREATE TABLE IF NOT EXISTS rc_algo.dm_rc_user_region_streamer_country_preference_detail
(
...
#写入表
sql11 = """
INSERT OVERWRITE TABLE rc_algo.dm_rc_user_region_streamer_country_preference_detail PARTITION (dt='{0}')
SELECT
user_region_name,
is_user_vip,
...
from tmp_rc_user_region_streamer_country_rfm_rnk_percentage
group by 
user_region_name,
is_user_vip,
...
#主播id-主播国家-频次-时长-金额-最近一次播出时间-原值-分数-rfm综合分数-rfm综合排名-rfm综合百分比-等级
sql20 = """
CREATE TABLE IF NOT EXISTS rc_algo.da_rc_user_region_streamer_country_preference
(
user_region_name string comment '用户区域',
is_user_vip string comment '是否vip',
...
#写入表
sql21 = """
INSERT OVERWRITE TABLE rc_algo.da_rc_user_region_streamer_country_preference PARTITION (dt='{0}')
SELECT
user_region_name,
is_user_vip,
...
rfm_rnk,
preference
from rc_algo.dm_rc_user_region_streamer_country_preference_detail
where dt = '{0}'
group by 
user_region_name,
is_user_vip,
...
if __name__ == "__main__":
    feature_category = 'da_rc_user_region_streamer_country_preference'
    spark = SparkSession.builder.enableHiveSupport().appName(feature_category).getOrCreate()
    
    #tmp_rc_user_region_streamer_country_rfm_rnk_max
    tmp_rc_user_region_streamer_country_rfm_rnk_max = spark.sql(sql1)
    tmp_rc_user_region_streamer_country_rfm_rnk_max.createOrReplaceTempView("tmp_rc_user_region_streamer_country_rfm_rnk_max")
    tmp_rc_user_region_streamer_country_rfm_rnk_max.show(10)

    #tmp_rc_user_region_streamer_country_rfm_rnk_percentage
    tmp_rc_user_region_streamer_country_rfm_rnk_percentage = spark.sql(sql2)
    tmp_rc_user_region_streamer_country_rfm_rnk_percentage.createOrReplaceTempView("tmp_rc_user_region_streamer_country_rfm_rnk_percentage")
    tmp_rc_user_region_streamer_country_rfm_rnk_percentage.show(10)


    #数据写入hive
    spark.sql(sql10)
    spark.sql(sql11)
    print("***finish rc_algo.dm_rc_user_region_streamer_country_preference_detail***")

    spark.sql(sql20)
    spark.sql(sql21)
    print("***finish rc_algo.da_rc_user_region_streamer_country_preference***")
